<template>
  ...
  >
    ...
    <div class="fan-list-content" v-loading="loading">
      <!-- 用户列表 -->
...
const fetchFanList = async () => {
  ...
  try {
    ...
    // 如果传入了userId，则查询指定用户的粉丝列表
    if (props.userId) {
      params.userId = props.userId
    }

    const res = await FollowAPI.getFansList(params)
...
<style scoped>
.fan-list-modal :deep(.el-dialog__header) {
  margin: 0;
  padding: 16px 20px;
  border-bottom: 1px solid #f0f0f0;
}

.fan-list-modal :deep(.el-dialog__body) {
  padding: 0;
  max-height: 500px;
  overflow-y: auto;
}
